...
import { SimulateProposalBase, MerkleDistributorFactoryLike, MerkleDistributorLike, StreamVaultLike, SablierLike } from "../SimulateProposalBase.t.sol";
...
contract Proposal17Test is SimulateProposalBase {
    function test_proposal_17() public onlyFork {
        ...
        // address target = address(new Proposal17());
        address target = 0xC391947dD63fCB927B65F3A5D35b4aa0511Be048; // deployed
        bytes memory data = abi.encodeWithSelector(Proposal17.execute.selector);
        ...
        assertEq(distributor.merkleRoot(), 0x47776b1e037cf5439f1d3ccfc4b11ddf34b1ba3578342e5224aae1d32f57f5c0);
        assertEq(prot.balanceOf(address(distributor)), 0x010bde77935c2112b600);

        testStreamVault(
            0x0FA9c7Ad448e1a135228cA98672A0250A2636a47,
            5999 ether,
            0x0a453F46f8AE9a99b2B901A26b53e92BE6c3c43E
        );

        testStreamVault(
            0x2df62660E30b8C5Bb211cA11e6525c6DD5D43200,
            2999 ether,
            0x9640F1cB81B370186d1fcD3B9CBFA2854e49555e
        );
